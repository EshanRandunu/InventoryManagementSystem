...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* === NEW: logout button style (matches secondary but slightly different) === */
  .btn-logout {
  ...
  .btn-logout:hover {
    background: rgba(0,0,0,0.04); border-color: var(--ink-muted);
...
function UserProfile() {
  ...
  useEffect(() => {
    ...
    if (!userId) { alert("Please login first"); navigate("/signin"); return; }
    loadUser(userId);
  }, [navigate]);
  ...
  const logout = () => {
    if (window.confirm("Are you sure you want to log out?")) {
      localStorage.removeItem("userId");
      localStorage.removeItem("role");
      navigate("/"); // redirect to home page instead of signin
    }
  };
      ...
      <div className="up-root">
        ...
        <main className="up-content">
          ...
          <div className="up-right">
            ...
            {/* ===== Actions card with new logout button ===== */}
            <div className="actions-card">
              <button className="btn-secondary" onClick={() => navigate("/profile/edit/" + user.id)}>
                ✎ Edit Profile
              </button>
